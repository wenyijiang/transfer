	...
	if (time_step > 0)
	{
		if (!mExternRBDModel)
		{
			UpdateRBDModel();
                ...
                Eigen::VectorXd tau=Eigen::VectorXd(GetNumDof());           //attention  the use of eigen lib /////////////////////////////
		//Eigen::VectorXd tau;
...
void cImpPDController::CalcControlForces(double time_step, Eigen::VectorXd& out_tau)                     ////////////////////attention
{
       double t = time_step;
...
/////////////////////
//out_tau=Eigen::VectorXd(23);
	for (int j = 1; j < GetNumJoints(); ++j)
	{
		cPDController& pd_ctrl = GetPDCtrl(j);
		if (!pd_ctrl.IsValid() || !pd_ctrl.IsActive())
		{				
			out_tau[j+2]=0;
			continue;
		}
		if(mChar->GetJoint(j).IsValid())
		{
			out_tau[j+2]=pd_ctrl.CalcTorque(mChar->GetJoint(j))[2];												
		}	
		else
		{
			out_tau[j+2]=0;
